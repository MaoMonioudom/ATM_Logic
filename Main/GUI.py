...
def show_register(root):
    def register_user():
        ...
        if not balance.isnumeric():
            messagebox.showerror("Error", "❌ Warning: Only number allow !!")
            return 
        ...
        if password.isdigit() and len(password) == 4:
            ...
            messagebox.showerror("Error", "Password must be digit and has length of 4.")
...
def view_trans_history(user_id):
    transactions = []
    try:
        with open(TRANS, "r", newline='') as f:
            reader = csv.reader(f)
            for row in reader:
                if row and (row[1] == str(user_id) or row[3] == str(user_id)):
...
def show_view_trans_history(root, user_id):
    window = tk.Toplevel(root)  # Use Toplevel to create a new window, not a new Tk instance
    window.geometry("800x500")
    window.title("Transaction History")

    transactions = view_trans_history(user_id)  # Get transactions once

    columns = ['Sender', 'Sender ID', 'Receiver', 'Receiver ID', 'Transaction Type', 'Amount', 'Date']
    tree = ttk.Treeview(window, columns=columns, show="headings")

    # Set up headings and column widths
    for col in columns:
        tree.heading(col, text=col)
        tree.column(col, width=120)  # Set width for better display
    ...
    # Add Scrollbars
    vertical_scrollbar = tk.Scrollbar(window, orient="vertical", command=tree.yview)
    tree.config(yscrollcommand=vertical_scrollbar.set)
    vertical_scrollbar.pack(side="right", fill="y")

    horizontal_scrollbar = tk.Scrollbar(window, orient="horizontal", command=tree.xview)
    ...
    # Back Button
    exit_button = tk.Button(window, text="Back", command=window.destroy, width=8)
    exit_button.pack(pady=10)
